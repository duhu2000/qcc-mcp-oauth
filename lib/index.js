 ...
 *   - 一次授权覆盖企查查 MCP 企业数据 SERVER（company/risk/ipr/operation/history/executive，共 6 个；history 需企业认证，按 token 实际授权动态配置）
...
export async function apply(ctx, config) {
  ...
  async function fullConnect(server, signal) {
    ...
    // 按 token 实际授权范围过滤要配置的 server：企业认证账号 token 的 resource 含 history（6 个），
    // 个人账号不含（5 个）——只配置实际授权的条目，避免把未授权的 history 挂出来却调不通。
      ...
      try {
        ...
        if (state.grant) {
          if (state.grant.accessTokenExpiresAt > Date.now()) {
            ...
            return { ok: true, message: `企查查 MCP 已连接（复用现有授权，覆盖 ${keys.length} 个 Server：${keys.join('/')}）`, detail: { reused: true, servers: keys } };
          }
          try {
            await refreshGrant();
        ...
        return {
          ok: true,
          message: `企查查 MCP 授权成功，已连接 ${ids.length} 个 MCP Server（${keys.join('/')}），可以直接使用 mcp__qcc-* 工具`,
  ...
  if (!state.grant && config.autoConnectOnActivate) {
    logger.info('no valid QCC authorization found — auto-starting OAuth connect (disable with autoConnectOnActivate: false)');
    const signal = new AbortController().signal;
    api.connect('company', signal).then((result) => {
